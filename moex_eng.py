...
def sql_create_stm():
    target_table = "ISS_MOEX_ENGINES"
    address = 'https://iss.moex.com/iss/engines.json'
    data = urlopen(address)
    text = json.loads(data.read().decode())
    columnes=text['engines']['columns']
    type=[]
    for col in columnes:
        type.append(text['engines']['metadata'][col]['type'])
    type_adj = list(map(lambda item: item.replace("int32","INTEGER").replace("string", "VARCHAR"), type))
    bytes=[]
    for col in columnes:
        try:
            bytes.append(text['engines']['metadata'][col]['bytes'])
        except:
            bytes.append(None)
